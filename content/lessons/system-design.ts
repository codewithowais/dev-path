...
const lessons: Lesson[] = [
  {
    ...
    easy: "Picture a bucket that holds a few tokens. Each time you make a request, you spend one token. If the bucket is empty, you must wait — the system blocks you. The bucket slowly refills over time, one token at a time. So after a short wait, you can make requests again. This is a rate limiter. It lets you burst a lot of requests for a short time. But it stops anyone from hammering the system nonstop.",
    how: [
      "Give each user (or API key) a bucket with a maximum size — say, 3 tokens.",
      "When a request comes in, first refill the bucket a little. Base the refill on the time since the last check: elapsed time ÷ refill rate = new tokens. Never go above the bucket's capacity.",
      "If the bucket has at least one token, take one and allow the request. If the bucket is empty, block the request.",
    ],
    when: "Use a rate limiter when your app gets popular. One user, or one buggy script, could send your server thousands of requests a second. A rate limiter protects everyone else by capping how fast any single caller can go.",
    big: "O(1) time and space per request — you just update a couple of numbers, no matter how many requests you have handled.",
    mistakes: [
...
t=40: allowed (tokens left: 2)`,
  },
  {
    ...
    how: [
      ...
      "Sort the servers by position. They now form a ring around the circle.",
      "For any key, hash it to a position too. Walk clockwise to find the first server at or after that position. That server owns the key. If you reach the end of the circle, wrap back to the first server.",
    ],
    when: "Use this when you spread data or traffic across many servers, and you want adding or removing a server to be cheap. A distributed cache — like a Redis or Memcached cluster — is a good example: you don't want to reshuffle almost everything each time you scale up or down.",
    big: "O(n) to build the ring (n = number of servers) · O(n) per lookup in this simple version (real systems use a sorted structure for O(log n) lookups). The big win here is not speed. It's that scaling the cluster only moves a small slice of keys.",
    mistakes: [
...
Keys that moved: 1 out of 6`,
  },
  {
    ...
    easy: "Picture a huge pile of mail. Instead of one overflowing mailbox, you sort it into 4 mailboxes. Sharding does the same thing to your database. Instead of one giant table holding every user, you split it into several smaller tables called shards. Each user's data lives in exactly one shard. To pick the right shard for a piece of data, you turn its key (like a user ID) into a number. You use that number to pick a shard. The same key always lands in the same shard.",
    how: [
      "Pick a number of shards, N — for example, 4 database instances.",
      "For any key, like a user ID, compute a hash number from it. The same key always produces the same number.",
      "Take the hash mod N. That remainder — a number from 0 to N-1 — tells you exactly which shard owns that key.",
    ],
    when: "Use sharding when one database server can no longer hold all your data or handle all your traffic. You split the data across several servers, so each one deals with only a fraction of it.",
    big: "O(1) time to route any key to its shard — it's just a hash and a division. The tradeoff: if you later change the number of shards, most keys move to a different shard. Consistent hashing is designed to avoid that problem.",
    mistakes: [
      "Picking a shard count and then changing it casually. As this example shows, going from 4 shards to 5 moves most keys to a different shard. That means a painful data migration.",
      "Sharding by a key that isn't evenly spread out — for example, by signup date, when most users signed up in one week. This creates a 'hot shard' that gets overloaded while the others sit idle.",
    ],
    code: {
...
Resharding 4 -> 5 moves 3 of 6 keys`,
  },
  {
    ...
    how: [
      ...
      "Either way, each incoming request goes to whichever server the pattern points to next. No server needs to know anything about the others.",
    ],
    when: "Use a load balancer when you have more traffic than one server can handle. You run several copies of your app behind one address, and something needs to decide which copy handles each incoming request.",
...
Weighted round robin (10 requests, A gets 3x): server-A server-A server-A server-B server-C server-A server-A server-A server-B server-C`,
  },
  {
    ...
    easy: "Pub/Sub works like a radio station. The station (the publisher) broadcasts on a channel (a 'topic'). It doesn't know or care who's listening. Anyone tuned in (a subscriber) hears the broadcast. The publisher never talks directly to the listeners. A middleman (the 'broker') delivers the message to everyone who subscribed to that channel.",
    how: [
      "Subscribers register interest in a topic by name — for example, 'orders'.",
      "A publisher sends a message to a topic. It doesn't know or care who — or how many services — are subscribed.",
      "The broker looks up every subscriber for that topic. It delivers a copy of the message to each one.",
    ],
    when: "Use pub/sub when one event needs to trigger several unrelated things, without those things knowing about each other. For example, a new order should notify billing, email, and shipping — but the order-placing code shouldn't need to know the details of all three.",
    big: "O(s) time per publish, where s is the number of subscribers on that topic. Each subscriber gets one delivery.",
    mistakes: [
...
email-service: [orders] Order #101 placed | [shipping] Order #101 shipped | [orders] Order #102 placed`,
  },
  {
    ...
    easy: "A leaderboard is a scoreboard. It tracks everyone's score and always shows them ranked from highest to lowest. The tricky part is ties. You need a consistent tie-breaker — here, alphabetical order by name — so the ranking never wobbles. Whenever someone's score changes, you must recompute the ranking.",
    how: [
      "Keep a running score for every player in a lookup table: name maps to total score.",
      "To show the leaderboard, sort all the names. Sort by score first, highest to lowest. Break ties by name, alphabetically.",
      "When a player earns more points, add them to that player's running score. Then re-sort to get the fresh ranking.",
    ],
    when: "Use this for any 'top players,' 'most active users,' or 'trending posts' feature — anywhere you need to show things ranked by a number that keeps changing.",
    big: "O(n log n) time to produce a full ranking (n = number of players), because of the sort · O(n) space to hold the scores. Real large-scale leaderboards use smarter data structures, like sorted sets, to update a single score in O(log n) time — instead of re-sorting everything.",
    mistakes: [
...
4. dave - 65`,
  },
  {
    ...
    easy: "Think of a car license plate. It packs a state code, a year, and a serial number into one plate. No two cars ever collide, even ones made in different states at the same time. A Snowflake-style ID does the same trick for database rows. It combines three things into one number: the current time, which server made the ID, and a small counter. The counter ticks up if the same server makes more than one ID in the same instant. Combine all three, and every server can hand out IDs at the same moment — without ever handing out the same one twice.",
    how: [
      "Reserve some bits of the final number for a timestamp. Reserve more bits for a machine ID — which server made it — and the rest for a sequence counter.",
      "Shift the timestamp left, past the machine-ID and sequence bits. Shift the machine ID left, past the sequence bits. Then combine all three into one number with a bitwise OR.",
      "If two IDs are requested at the exact same timestamp, bump the sequence counter so they still come out different. When the timestamp changes, reset the counter to 0.",
    ],
    when: "Use this when you generate IDs across many servers at once, so you can't just use an auto-incrementing database column. You still want IDs that sort roughly by creation time. This is common in large-scale systems, like chat messages, orders, or analytics events.",
    big: "O(1) time and space per ID generated — just a few shifts and an OR.",
    mistakes: [
      "Reading the real system clock inside the core logic. This makes the code hard to test, and the clock can even go backwards if the server's clock gets adjusted. Instead, pass the timestamp in as a value, like this example does.",
      "Forgetting to reset the sequence counter when the timestamp moves forward. Without the reset, the counter keeps climbing forever instead of starting fresh each 'tick.'",
    ],
    code: {
...
All IDs unique: yes`,
  },
  {
    ...
    how: [
      ...
      "Write-through: on every write, update the cache and the database right away, before telling the caller 'done.' Reads always match between the two.",
      "Write-back: on every write, update only the cache, and mark that entry 'dirty' — changed but not yet saved. A later step, called a 'flush,' copies all dirty entries into the database and clears the dirty marks.",
      "Reads always check the cache first. If the data isn't there, fall back to the database.",
    ],
    when: "Use this once your app is popular enough that hitting the real database on every write slows everything down. Write-through is worth the extra cost when you can never afford to lose a write — like a bank balance. Write-back is worth the risk when writes happen often, and a short window of possible data loss is acceptable in exchange for speed — like counting 'likes' on a post. That data loss could happen if the server crashes before flushing.",
    big: "O(1) time per cache read or write in both strategies. Write-through pays one extra O(1) database write on every single write. Write-back puts off that cost. It flushes many writes to the database in one batch, later. That's faster now, but anything still 'dirty' is lost if the cache disappears before the flush.",
    mistakes: [
...
database (after flush): user:1=Alicia, user:2=Bob, user:3=Carol`,
  },
  {
    ...
    easy: "You've probably clicked a 'Pay Now' button twice because the page seemed frozen. Then you worried you had just been charged twice. An idempotency key fixes that. It's a unique ticket number the app attaches to that one purchase attempt. The server keeps a log of ticket numbers it has already handled. Say the same ticket number shows up again, because the click — or a network retry — got sent twice. The server doesn't charge you again. It just hands back the same result as last time, as if it never ran the action twice.",
    how: [
      "Before sending a request that changes something, like 'charge this card,' the client generates one unique key for that attempt. It attaches the key to the request.",
      "The server checks a lookup table: has this exact key been seen before? If yes, skip the action. Just return the result saved from the first time.",
      "If the key is new, perform the action for real. Save the key and its result together. Then return the result.",
    ],
    when: "Use idempotency keys once your app is popular enough that flaky networks, slow pages, or retry logic start sending the same request more than once. Payments, placing an order, and sending a signup email are exactly the kind of action you never want to run twice by accident.",
    ...
    mistakes: [
      "Generating a brand-new key every time you retry, instead of reusing the same key for the same attempt. That defeats the whole point — the server can no longer tell it's a repeat.",
      "Remembering only that 'this key was used,' without saving the actual result. Then a retry can't get back the original answer it needs, like the order ID.",
    ],
    code: {
...
Total real charges made: 2`,
  },
  {
    ...
    easy: "Picture a teacher — the 'primary' — writing notes on the whiteboard. Several students — the 'replicas' — copy those notes into their own notebooks. That way everyone can read them without crowding around the board. Only the teacher writes new notes; students just copy. That's replication. One primary database handles all the writes, and streams every change to one or more replica copies. Spreading reads across the replicas lets far more people read at once. But a slow-copying student's notebook might briefly lag behind what's on the board. This is called replication lag.",
    how: [
      "Every write goes to exactly one server: the primary. It applies the change, and appends it, in order, to a running log of everything that changed.",
      "Each replica reads that log and applies the same changes, in the same order, to its own copy of the data.",
      "Reads get spread across the replicas, instead of all hitting the primary. This way, read traffic doesn't overload the one server that also handles writes. A replica that hasn't caught up yet may return slightly stale — out-of-date — data.",
    ],
    when: "Use this once your app is popular and has far more reads than writes — like a blog, a product catalog, or a news feed. Add several read replicas, so all that read traffic doesn't pile onto the one server that's busy handling writes.",
    big: "O(1) time to append one write to the primary's log · O(k) time for a replica to catch up on k unapplied log entries.",
    mistakes: [
      "Sending a write straight to a replica. Replicas are meant to be read-only copies. Writing to one breaks the whole replication setup.",
      "Assuming a replica is always instantly up to date. Reading from a lagging replica right after a write can show you the old value. This is called the 'read-your-own-writes' problem.",
    ],
    code: {
...
Replica B has applied 2 of 4 writes — reads from it may be stale.`,
  },
  {
    ...
    easy: "Imagine calling a friend every few minutes to check in, while they do something risky alone. As long as they keep answering, you assume they're fine. But if a few calls in a row go unanswered, you start to worry. This is exactly how servers watch each other. Every server sends a tiny 'I'm alive' ping — a 'heartbeat' — on a regular schedule. A monitor tracks the last time it heard from each one. If too much time passes with no ping, the monitor marks that server 'down.' It assumes the server has failed.",
    how: [
      ...
      "Whenever you check a server's status, compare the current time to its last heartbeat time. If more time has passed than the allowed timeout, mark it 'down.' Otherwise, it's 'up.'",
    ],
    when: "Use this once your app is popular enough that you run many servers, and you need to automatically notice when one crashes. Then a load balancer can stop sending it traffic, or an alert can fire. This is how real clusters — like Kubernetes watching its nodes — know something has failed, without a human checking by hand.",
...
t=20: node-1=down, node-2=down`,
  },
  {
    ...
    easy: "Picture a bucket with a small hole in the bottom. It drips water out at a steady rate, no matter how fast you pour water in. Requests are like water being poured in. If they arrive faster than the hole can drain them, the bucket fills up. Once it's full, any extra water spills over the top and is lost — the request gets blocked. This differs from the token bucket rate limiter. A token bucket lets you save up tokens while idle, then use them all in one big burst. A leaky bucket always lets requests out at the same steady drip. No bursts, ever.",
    how: [
      "Keep a queue — the bucket — with a maximum size, and a fixed 'leak rate.' The leak rate is how often one waiting request drains out and gets processed.",
      "When a new request arrives, first drain out any requests that should already have leaked, based on how much time has passed.",
      "If there's room left after draining, add the new request to the back of the queue. It will be processed in its turn. If the bucket is still full, reject the request — it spills over.",
    ],
    when: "Use a leaky bucket once your app is popular and needs to call a downstream service — like a payment gateway or a partner API — that can only handle a steady stream of requests, and would choke on even a short burst. A leaky bucket smooths out bursty incoming traffic into one constant, predictable rate. A token bucket, by contrast, would let a burst straight through.",
    big: "O(1) time per request, averaged over many requests · O(capacity) space to hold the queued requests.",
    mistakes: [
      "Mixing this up with the token bucket. The token bucket is built to allow bursts. The leaky bucket is built to flatten bursts into one steady rate.",
      "Forgetting to drain old requests before checking whether there's room. That makes the bucket look fuller than it is, and rejects requests that should have been allowed.",
    ],
    code: {
...
t=16: blocked (queue size: 3)`,
  },
  {
    ...
    easy: "Before a careful accountant erases a number on a whiteboard and writes the new one, they first jot the change in a permanent notebook: 'changing balance from 100 to 80.' If the power goes out right after the whiteboard is half-erased, the notebook still says exactly what was supposed to happen. The accountant can finish the job later without losing track. A write-ahead log (WAL) does this for a computer system. Before changing the real data, it first appends a record of that change to a durable, append-only log. If the process crashes mid-change, replaying the log rebuilds the data exactly as it should be. Nothing is lost.",
    how: [
      "Before applying any change to the actual data, first append the change as a record to the write-ahead log — a durable, append-only file. This record describes exactly what is about to happen.",
      "Only after the log record is safely written, apply the same change to the real data store.",
      "If the system crashes before finishing, replay every record in the log from the start on restart. Reapply each change. This reconstructs the data exactly as it should be, even though the crash happened mid-work.",
    ],
    when: "Use a write-ahead log once your app can't afford to lose data if the server crashes mid-write. A database engine, a bank ledger, and a message queue all use one. As long as the log write succeeded, you can always recover the change after a crash — even if the actual data file never got updated.",
    big: "O(1) time to append one log record · O(n) time to replay n log records during recovery.",
    mistakes: [
      "Applying the change to the data store before — or instead of — writing it to the log. If a crash happens in between, that change is lost forever. This defeats the entire purpose of a WAL.",
      "Forgetting to actually replay the log on restart. The log only protects you if something reads it back and rebuilds the data from it after a crash.",
    ],
    code: {
...
data: balance:alice=80, balance:bob=50`,
  },
  {
    ...
    easy: "Think of a restaurant kitchen and its menu board. The kitchen — the 'write side' — is built to correctly cook complicated orders, one at a time. The menu board — the 'read side' — is a simple, quick-to-scan summary customers glance at. Nobody walks into the kitchen to check what's available. Whenever the kitchen changes something, it also updates the board. CQRS splits a system the same way. CQRS stands for Command Query Responsibility Segregation — a fancy way of saying 'writing and reading are two different jobs.' Commands (do this — place an order) go through a model built for correctness. Queries (tell me this — show my order history) go through a separate, simpler model built for fast reading.",
    how: [
      "Keep two models. The 'write model' handles commands — create, update, delete — and enforces the real rules. The 'read model' is a simplified, ready-to-read copy, shaped exactly for what queries need.",
      "Every time a command changes the write model, also update the read model — this step is called 'projecting' the change — so it reflects the update.",
    ...
    when: "Use CQRS once your app is popular enough that how you write data doesn't match how you read it. Writing is careful, one record at a time, with lots of checks. Reading needs fast summaries across many records. For example, in an order system, placing an order is complex, but showing 'orders per customer' needs to be instant. CQRS lets you optimize each side on its own, instead of forcing one model to do both jobs well.",
    big: "O(1) time to apply a command to the write model · O(1) time to project a change into the read model · queries become O(1) lookups on an already-built structure, instead of recomputing summaries from scratch every time.",
    mistakes: [
      "Letting a query read straight from the write model 'just this once.' That brings back the tight link CQRS is meant to remove, and the fast read model quietly drifts out of sync.",
...
bob: 2 orders, total $60`,
  },
  {
    ...
    how: [
      ...
      "As the consumer drains items from the buffer, the buffer shrinks. Once it drops back to a low-water mark — a lower threshold — tell the producer it's safe to resume pushing.",
    ],
    when: "Use backpressure once your app is popular and a fast producer — like an API taking in incoming events — can generate work faster than a slower consumer, like a database writer, can keep up. Without backpressure, the buffer between them grows without limit, and the whole process eventually runs out of memory and crashes.",
    ...
    mistakes: [
      "Letting the buffer grow with no maximum size at all. That only delays the crash instead of preventing it, since memory use is still unlimited.",
...
produce e7: accepted (buffer size: 2)`,
  },
  {
    ...
    easy: "Think of a bouncer at a club with a guest list, checking off names as people enter. If the same name tries to walk in twice — maybe they wandered off and came back — the bouncer sees the name is already checked off. They turn the person away the second time. Only the first entry counts. Deduplication does this for events flowing through a system. It remembers which events — by a unique ID — it has already handled. It silently drops any repeat, instead of processing it again.",
    how: [
    ...
    mistakes: [
      "Deduplicating by the event's content instead of a unique ID. Two genuinely different events can have identical content — like two separate $10 purchases. Content-based dedup would wrongly drop the second, real one.",
