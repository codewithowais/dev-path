...
const lessons: Lesson[] = [
  {
    ...
    easy: "HTTP (HyperText Transfer Protocol) is the language browsers and servers use to talk to each other. Think of ordering food at a restaurant. You, the browser, tell the waiter what you want. That's a request. The kitchen, the server, makes it. The waiter brings back a response, either what you asked for, or an apology that it's not available. Every time you open a web page, your browser sends a request and waits for a response.",
    how: [
      "The client (your browser or an app) builds a request. It picks a method, like GET, and a path, like /home.",
      "The request travels to a server. A server is a computer whose whole job is to listen for requests and answer them.",
      "The server looks at the request and decides what to do. It builds a response: a status that says whether it worked, plus a body with the actual content.",
      "The response travels back to the client. The client reads it and shows you the result — a page, an error, or whatever it received.",
    ],
    when: "This happens every time a web page loads, every time you click a button that fetches new data, and every time a mobile app pulls your feed. It happens anywhere two computers exchange information over the web.",
    mistakes: [
      "Some people think the server 'pushes' pages to you without being asked. Normally the client always asks first, and the server only ever replies.",
      "Don't forget that each request and response usually stands alone. The server doesn't automatically remember your last request, unless something like a cookie (a small piece of stored data) reminds it who you are.",
    ],
    code: {
...
Response: 404 Not Found`,
  },
  {
    ...
    easy: "A status code is a three-digit number the server attaches to every response. It's like a waiter's one-word summary of how your order went: 'here you go' (200), 'we don't have that' (404), or 'the kitchen caught fire' (500). You don't have to read the whole response to know if it worked. The status code tells you right away.",
    how: [
      "Codes that start with 2 mean success. The request worked, like 200 OK.",
      "Codes that start with 4 mean the client made a mistake. For example, asking for something that doesn't exist gives 404 Not Found.",
      "Codes that start with 5 mean the server had a problem while trying to help. For example, it crashed, giving 500 Internal Server Error.",
      "Real programs check the status code first. Then they decide what to do next: show the page, show an error, or try again later.",
    ],
    when: "Every HTTP (HyperText Transfer Protocol) response includes a status code. You'll see them often in browser dev tools, API (Application Programming Interface) responses, and server logs whenever something needs debugging.",
    big: "O(1) lookup — checking a status code means reading one number. It takes the same amount of time no matter how big the rest of the response is.",
    mistakes: [
      "Don't assume a page that 'loads' always means success. A 404 or 500 page still loads fine in the browser — it just carries bad news.",
      "Beginners often mix up 401 and 403. 401 (Unauthorized) means you're not logged in. 403 (Forbidden) means you're logged in but not allowed.",
    ],
    code: {
...
500 (server error): Internal Server Error - the kitchen caught fire`,
  },
  {
    ...
    easy: "The client is the customer. The server is the kitchen. The client (a browser, a phone app, or another program) sends requests asking for things. The server is a computer that's always running and listening. It does the actual work and sends results back. The client never does the work itself. It always asks the server and waits for the answer.",
    how: [
      "The client puts together a request that describes what it wants.",
      "The client sends the request and waits for a response.",
      "The server is always running and listening. It receives the request and does the actual work.",
      "The server sends its result back as a response. The client then uses it, for example by showing a page or updating the screen.",
    ],
    when: "This happens anytime an app needs data or work done somewhere else. Examples: loading a webpage, checking your email, or a mobile app fetching your latest notifications.",
    mistakes: [
      "Don't assume 'server' always means a giant machine in a data center. It can be a laptop, a phone, or a small program. The point is that it waits for and answers requests.",
      "Don't forget that the client starts the exchange. Over plain HTTP (HyperText Transfer Protocol), the server can't just decide to send you something out of nowhere.",
    ],
    code: {
...
Client receives: order #42 is ready`,
  },
  {
    ...
    easy: "DNS (Domain Name System) is the internet's phone book. Computers don't actually go to 'example.com'. They go to a numeric address called an IP (Internet Protocol) address, like 93.184.216.34. DNS is the lookup service that turns the easy-to-remember name you type into the number your computer needs to connect.",
    how: [
      "You type a domain name, like example.com, into your browser.",
      "Your computer asks a DNS server: 'What is the IP address for this name?'",
      "The DNS server looks it up in its records and returns the matching IP address.",
      "Your browser then connects directly to that IP address to load the page.",
    ],
    when: "This happens every time you visit a website by typing its name instead of a raw IP address. That's almost always, since nobody memorizes strings of numbers.",
    big: "O(1) lookup in a simple table like ours. Real DNS involves several hops between servers, but it works the same way underneath.",
    mistakes: [
      "Don't forget that DNS results can be cached, meaning remembered for a while. That's why a site can 'still work' briefly right after its DNS record changes, or why a change can take time to spread to everyone.",
      "Don't confuse a domain name with the website itself. The domain is just a label. DNS's only job is to point that label at an address.",
    ],
    code: {
...
unknown.test -> not found`,
  },
  {
    ...
    easy: "An API (Application Programming Interface) is the menu of things a server lets other programs ask for. A REST API organizes requests by combining a method, like GET or POST, with a path, like /users/1. Routing is the server's job of matching that combination against a list of known routes, then calling the right handler. It's like a receptionist reading your appointment slip and pointing you to the right department.",
    how: [
      "The server keeps a list of routes it knows about. Each one pairs a method and path pattern with a handler function.",
      "When a request comes in, split its path into pieces and compare them to each route's pattern, piece by piece.",
      "A piece like :id in the pattern is a placeholder. It matches anything and captures that value as a parameter.",
      "The first route that fully matches handles the request. If nothing matches, the server answers with a 404.",
    ],
    when: "This applies when building any REST API, a common style for web services. Each resource gets one URL (Uniform Resource Locator) path, and each action gets one method, like GET /users/1 to fetch and POST /users to create.",
    big: "O(number of routes) to find a match in this simple version. Real frameworks use faster lookup structures, but the matching idea is the same.",
    mistakes: [
      "Don't forget that the method matters as much as the path. GET /users and POST /users are two different routes, even though the path looks the same.",
      "Watch the order of routes when patterns overlap. A very general pattern placed before a specific one can accidentally catch requests meant for the specific one.",
    ],
    code: {
...
DELETE /users/1 -> 404 Not Found`,
  },
  {
    ...
    easy: "A URL (Uniform Resource Locator) is the web address you type or click. It often carries extra information after a question mark, called a query string, like /search?q=cats&limit=10. Parsing means splitting that text into a clean set of key-and-value pairs your program can use. It's like splitting a shopping list written as one long sentence into separate items.",
    how: [
      ...
      "Collect all the pairs into a simple lookup, like an object or dictionary, so your code can read params.q or params.limit directly.",
    ],
    when: "This applies when reading filters, search terms, page numbers, or IDs passed in a web address. Almost every search bar, filter dropdown, or 'page=2' link relies on query parsing.",
    mistakes: [
      "Don't forget a URL might have no query string at all. Always check for the '?' before assuming there's anything to split.",
      "A real parser must decode special characters, like %20 for a space. Our simplified version skips this, but production code must handle it.",
    ],
    code: {
...
limit = 10`,
  },
  {
    ...
    easy: "JSON (JavaScript Object Notation, said 'JAY-son') is a simple text format for sending structured data between programs. It's like a filled-out form that sends information in labeled fields, instead of one long paragraph. It represents data as key-and-value pairs, like \"name\": \"Ada\", as lists, like [\"math\", \"code\"], and as nested groups of both. This lets any two programs, in any language, agree on the shape of the data passing between them.",
    how: [
      "A program on one end turns its data into JSON text, ready to send anywhere. This step is called 'serializing'.",
      "That text travels over HTTP (HyperText Transfer Protocol), gets saved to a file, or moves however it needs to.",
      "A program on the other end reads the JSON text and turns it back into real data it can use. This step is called 'parsing'.",
      "Once parsed, you read individual fields the normal way, like a value inside an object or list you already know how to use.",
    ],
    when: "Almost every modern API (Application Programming Interface) sends and receives JSON. It's the standard shape of data flowing between a client and a server on the web.",
    mistakes: [
      "Don't forget that JSON keys and strings must use double quotes, not single quotes. Text that breaks this rule simply fails to parse.",
      "Don't compare JSON text directly between two systems to check if 'the data is the same'. Always parse it first and compare the actual values, since formatting like spacing or key order can differ even when the meaning is identical.",
    ],
    code: {
...
city = London`,
  },
  {
    ...
    how: [
      "On your first visit, you have no wristband yet. The server has no way to tell you apart from a stranger.",
      "When you log in, the server creates a session record — who you are and what you're allowed to do — and stores it under a fresh session ID.",
      "The server tells your browser to remember that session ID as a cookie, and to attach it to every future request to this site.",
      "Your browser automatically resends the cookie on the next request, without you doing anything.",
      "The server reads the cookie, looks up that session ID in its records, and instantly knows who is asking.",
    ],
    when: "This applies to login systems, shopping carts, 'remember me' features, and anything else that needs to recall who you are across separate requests. Plain HTTP requests are otherwise independent, with no memory between them.",
    big: "O(1) session lookup — finding a session by its ID takes one step, a single hash-map lookup, no matter how many users are logged in.",
    mistakes: [
...
Visit 3, wrong cookie: guest (unknown session)`,
  },
  {
    ...
    when: "This applies to stateless APIs (Application Programming Interfaces) that don't want to store per-user session data, mobile app logins, and microservices that need to verify who's calling without sharing a session database between them.",
    big: "O(n) to sign or verify a payload of length n — verifying costs exactly as much work as signing did.",
    mistakes: [
...
Wrong secret accepted: no`,
  },
  {
    ...
    how: [
      ...
      "Next time the browser wants that same resource, it sends the ETag it already has along with the request, instead of assuming it needs a fresh copy.",
    ...
    when: "This applies to images, stylesheets, scripts, and API (Application Programming Interface) responses that don't change on every request — anywhere re-downloading identical data would waste bandwidth and time.",
...
Third request (content changed) status: 200 etag: etag-2062 body: Welcome home! (updated)`,
  },
  {
    ...
    how: [
      ...
      "The browser attaches the calling page's origin to the request, so the server can see exactly who is asking.",
    ...
    when: "This applies to any web app whose JavaScript calls an API (Application Programming Interface) hosted on a different domain or subdomain than the page itself. That's the normal setup for a modern single-page app talking to a separate API server.",
    big: "O(1) per request — the check just tests whether one origin string appears on an allow-list.",
    mistakes: [
...
Origin: https://evil.test -> Access-Control-Allow-Origin: none -> page can read response: no`,
  },
  {
    ...
    easy: "Regular HTTP (HyperText Transfer Protocol) is like exchanging letters by mail. Every time you want to say something new, you seal a fresh envelope, a brand-new request, and wait for a reply before you can send the next one. A WebSocket is a connection that, once opened, stays open in both directions. It's like hanging up the letters and picking up the phone instead. You dial once, and after that, either side can just speak whenever it wants, without hanging up and redialing for every single sentence.",
    how: [
      "The client sends a special HTTP request asking to 'upgrade' the connection into a WebSocket.",
      "The server agrees, and from that moment on, the same connection stays open for both sides to use.",
    ...
    when: "This applies to chat apps, live notifications, multiplayer games, live sports scores, and collaborative editing — anywhere the server needs to push updates the instant something happens, rather than waiting to be asked.",
    ...
    mistakes: [
      "Don't assume WebSockets replace HTTP entirely. Most apps still use regular HTTP requests for most things, and only add a WebSocket connection for the specific parts that need instant, two-way updates.",
      "Don't forget the connection can close unexpectedly — a dropped network, a server restart. Real code has to detect a closed connection and reconnect, or messages sent afterward simply go nowhere.",
    ],
    code: {
