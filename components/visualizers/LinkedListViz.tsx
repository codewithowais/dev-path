...
/** A random list of 3–4 distinct round-ish values, with a derived insert. */
function randomConfig(seed: number): LLConfig {
  const rng = makeRng(seed);
  const n = 3 + Math.floor(rng() * 2); // 3 or 4 nodes
  const values: number[] = [];
  const used = new Set<number>();
  while (values.length < n) {
    const v = (1 + Math.floor(rng() * 9)) * 5 + Math.floor(rng() * 5); // 5..49-ish
    if (!used.has(v)) {
      used.add(v);
      values.push(v);
    }
